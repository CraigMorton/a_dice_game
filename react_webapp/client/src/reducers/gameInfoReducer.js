...
const gameInfoReducer = (state = defaultState, action) => {
  const mapActionStringToActionDispatchCall = {
    ...
    "REMOVE_SHARED_RESOURCE_FROM_POOL": () => ({...state, sharedResource: {count: state.sharedResource.count - 1} }),
    "RESET_SHARED_RESOURCE_POOL": () => ({...state, sharedResource: {count: state.sharedResource.max} })
  }
